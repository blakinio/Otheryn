...
def read_routing(path: Path) -> dict[str, list[str]]:
    ...
    routing = {key: [] for key in ROUTING_KEYS}
...
def bundle(path: Path) -> dict[str, object]:
    ...
    keys = (
        "head", "branch", "pr", "status", "context_routes", "proven", "derived", "unknown",
        "conflicts", "first_failure", "changed_paths", "validation", "blockers", "next_action",
    )
    return {
        "task_id": task_id(path),
        "checkpoint": str(path),
