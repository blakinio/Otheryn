...
class ViewerRuntimeTests(unittest.TestCase):
	def test_modes_state_persistence_and_bounded_cache(self):
...
console.log(JSON.stringify({{parsed:['auto','detailed','performance'].map(v=>r.urlRenderMode('https://x/?render='+v)),invalid:r.resolveRenderMode('https://x/?render=no','bad'),precedence:r.resolveRenderMode('https://x/?render=performance','detailed'),layers:[r.baseLayer('detailed',.03),r.baseLayer('performance',4),r.baseLayer('auto',.1),r.baseLayer('auto',1)],switched,cache:[cache.size,cache.bytes,cache.get('b'),cache.get('a'),cache.get('c')],persist:r.persistMode(storage,history,'https://x/?x=1#y=2','detailed'),stored:storage.value,url:history.url,restored,visible:visible.length}}));'''
