...
class ViewerTests(unittest.TestCase):
	def test_static_viewer_exposes_navigation_and_required_overlays(self)->None:
		...
		self.assertIn('min="0" max="15"',text);app=(Path(__file__).parents[1]/"viewer_app.js").read_text(encoding="utf-8");runtime=(Path(__file__).parents[1]/"viewer_runtime.js").read_text(encoding="utf-8");self.assertIn("Number.isInteger(requestedZ)",app);self.assertIn("displayFloor=z=>z",app);self.assertIn("state.overlays=[...state.overlays,item.kind]",app);self.assertIn("if(target)showDetails(item.kind,target)",app);self.assertIn("for(const kind of state.overlays){const records=entry.value[kind]",app);self.assertIn("parseCoordinateSearch",app);self.assertIn("showTooltip",app);self.assertIn("drawNpcSprite",app);self.assertIn("record.sprite",app);self.assertIn("supplementalNpcSpawns",runtime);self.assertIn("supplementalMonsterSpawns",runtime);self.assertIn("p.has('z')&&Number.isFinite(+p.get('z'))?+p.get('z'):7",runtime)
