...
<label>Floor <select id="floor"></select></label><div><input id="x" type="number" placeholder="X"><input id="y" type="number" placeholder="Y"><input id="z" type="number" min="-8" max="7" placeholder="Floor"><button id="jump">Jump</button></div>
...
function displayFloor(rawZ){return 7-rawZ} function rawZ(display){return 7-display}
...
canvas.onpointerdown=e=>{drag=[e.clientX,e.clientY,centerX,centerY];canvas.setPointerCapture(e.pointerId)};canvas.onpointermove=e=>{const [wx,wy]=screenToWorld(e.clientX*devicePixelRatio,e.clientY*devicePixelRatio);status.textContent=`X ${Math.floor(wx)} Y ${Math.floor(wy)} Floor ${displayFloor(z)}`;if(drag){centerX=drag[2]-(e.clientX-drag[0])*devicePixelRatio/(32*scale);centerY=drag[3]-(e.clientY-drag[1])*devicePixelRatio/(32*scale);draw()}};canvas.onpointerup=()=>drag=null;
canvas.onwheel=e=>{e.preventDefault();scale=Math.max(.03,Math.min(4,scale*Math.exp(-e.deltaY*.001)));draw()};
floorSelect.onchange=()=>{z=+floorSelect.value;draw()};document.querySelector('#jump').onclick=()=>{centerX=+document.querySelector('#x').value||centerX;centerY=+document.querySelector('#y').value||centerY;const jumpFloor=document.querySelector('#z').value;if(jumpFloor!==''){z=rawZ(Math.max(-8,Math.min(7,+jumpFloor)));floorSelect.value=z}draw()};
