...
preconditions: [isolated party, deterministic boss health/time controls]
steps:
...
artifacts: [soulwar-state-machines.jsonl, boss-stats.csv, reset-results.json]
cleanup: [reset arenas and discard party state]
safety:
  production_access: false
  persistent_live_state: false
  external_side_effects: false
blocker: numeric official values require a primary reference; structural pass/fail is feasible
```
...
confidence: high
rationale: the source defines four falsifiable ordering/state defects, but code-path assignment and numeric balance require isolated execution
```
