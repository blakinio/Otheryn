...
preconditions: [isolated players at each Barkless stage and resettable Leiden arena]
steps:
...
artifacts: [barkless-dialogue-matrix.json, storage-timeline.jsonl, leiden-combat-events.jsonl]
cleanup: [discard players and reset arena]
safety:
...
confidence: high
rationale: the source identifies two concrete, independently testable mechanisms; dialogue/storage and combat-heal fixes must remain separate
```
