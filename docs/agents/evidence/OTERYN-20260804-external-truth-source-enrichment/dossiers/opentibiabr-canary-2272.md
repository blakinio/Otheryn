...
blocker: no authoritative formula/version or complete combat vector; reproducing “low hits” without an expected distribution
  cannot yield pass/fail
```
...
confidence: high
rationale: the report omits nearly every parameter that determines weapon damage and provides neither raw samples nor a versioned
  authoritative calculator/formula; any proposed code change would be tuning by guess Runtime execution is reference-blocked
