...
preconditions:
- isolated players before/at/after the Scorpion mission
steps:
...
artifacts:
- scorpion-tile-dump.json
- entry-matrix.json
- arena-state.jsonl
- runtime-feasibility.md
cleanup:
- reset arena and discard players
safety:
...
confidence: high
rationale: 'exact coordinate, item and action ID make the primary blocker deterministic; map/action/storage tracing is required
  before repair Runtime execution is infrastructure-blocked: the repository has no deterministic game/client driver and adding
  one is outside audit-only authority.'
```
