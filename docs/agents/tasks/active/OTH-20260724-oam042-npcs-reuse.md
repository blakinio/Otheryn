...
updated_at: 2026-07-24T09:45:00+02:00
head: 7c54172adfa612fa143d11630f5a341ff4c82338
branch: dudantas/oam-042-npcs-reuse-proof
status: implementing
context_routes:
  - agent-governance
...
derived:
  ...
  - A target-local source-contract test can protect the reviewed semantics without copying or rebuilding the external OTBM scanner.
unknown:
  - Full exact active-root inventory relationship for every NPC definition outside the reviewed representative files.
  - Whether both Harlow definitions reported by the external scanner are active under the exact target configuration.
  - Exact NPC-owned subset of nonliteral dynamic creation and quest-hook calls.
  - Whether source-contract plus reused placement evidence is sufficient for final REUSE or one bounded runtime dialogue/shop proof is still required.
conflicts: []
first_failure:
  marker: none
  evidence: No concrete NPC-owned target defect has been isolated.
rejected_hypotheses:
...
blockers: []
next_action: Add the bounded proof document and target-local source-contract test, register it once in the existing unit-test target, then run focused and final milestone validation.
```
