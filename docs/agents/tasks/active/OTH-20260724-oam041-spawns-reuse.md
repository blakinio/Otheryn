...
updated_at: 2026-07-24T00:25:00+02:00
head: 0c5ae138687d072eb84a3b58560b0721e695b41b
branch: dudantas/oam-041-spawns-reuse
pr: 85
status: validating
context_routes:
...
proven:
  ...
  - Exact successful deterministic proof run 30049543113 executed on target head 38bf8dcb4d401fa4053e350af052cc2e9c8da4bd using pinned Canary commit d1ad83056ec7930f067986909f66b8f20f1a1f44 and exact Phase 4 tool blobs.
  ...
  - The temporary proof workflow was removed after artifact capture; the delivered branch now has only the intended four proof/task paths and no production runtime datapack map binary protocol client schema or deployment mutation.
derived:
  - Final OAM-041 target disposition is spawns REUSE if exact-final-head repository gates remain green.
  - The known Harlow ambiguity and unresolved dynamic Lua calls are explicit evidence boundaries, not authorization for guessed or broad source repair.
...
unknown:
  - Exact final target CI Required and platform-gate evidence until the final four-path head finishes validation.
  - Final comment review review-thread and target-main-drift state at merge time.
conflicts: []
first_failure:
  marker: none
  evidence: No concrete spawns-owned target defect was isolated. Earlier proof-harness failures were resolved by narrowing the region and raising the reachability diagnostic sample bound without changing target production or data paths.
rejected_hypotheses:
...
validation:
  ...
  - command: immutable-base changed-path audit before final gate
    result: PASS
    evidence: temporary workflow removed; current intended delivered scope is exactly four proof/task paths with no production mutation
blockers: []
next_action: Run exact-final-head Otheryn CI and Required gates on PR 85, audit changed paths comments reviews review threads and target-main drift, then mark ready and expected-head squash merge if all gates remain clean.
```
