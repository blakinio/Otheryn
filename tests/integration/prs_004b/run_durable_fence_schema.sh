...
verify_contract() {
	...
	require_equal "1" "$(scalar "SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'player_writer_fence' AND CONSTRAINT_TYPE = 'PRIMARY KEY'")" "subject primary key"
