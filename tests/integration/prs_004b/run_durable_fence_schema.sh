...
export MARIADB_PWD="${TEST_DB_PASSWORD:-root}"
...
verify_contract() {
	...
	require_equal "$(scalar "SELECT COUNT(*) FROM players")" "$(scalar "SELECT COUNT(*) FROM player_writer_fence")" "one authority row per player"
...
# The workflow supplies one fresh disposable database.
"${mysql_base[@]}" -e "CREATE DATABASE IF NOT EXISTS \`${database}\` CHARACTER SET utf8 COLLATE utf8_general_ci"
